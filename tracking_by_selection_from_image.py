...
while True:
    ...
    # Draw crossed lines at the center of the frame
    draw_cross_lines(display_frame, center, (rect_start, rect_end), line_length_cm=0.25, pixels_per_cm=pixels_per_cm)
        ...
        if tracking_initialized:
            ...
            if success:
                ...
                if new_frame_time != prev_frame_time:  # Check for zero division
                    ...
                    # Display FPS information on the frame
                    cv2.putText(display_frame, f"FPS: {fps}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                    ...
                    cv2.putText(display_frame, "Object Detected", (bbox[0], bbox[1] - 10), cv2.FONT_HERSHEY_SIMPLEX,
                                0.5, (0, 0, 255), 1)

                    # Calculate the center coordinates of the tracking frame
                    tracked_object_center_x = int(bbox[0] + bbox[2] / 2)
                    tracked_object_center_y = int(bbox[1] + bbox[3] / 2)

                    # Display the center coordinates of the tracking frame
                    cv2.putText(display_frame, f"Center: ({tracked_object_center_x}, {tracked_object_center_y})",
                                (bbox[0], bbox[1] + bbox[3] + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

                    # Calculate the radius for a circle with a diameter of 1 cm
                    circle_radius = int(pixels_per_cm / 2)

                    # Draw a circle at the center of the tracking frame
                    cv2.circle(display_frame, (tracked_object_center_x, tracked_object_center_y), circle_radius,
                               (255, 0, 0), 1)
