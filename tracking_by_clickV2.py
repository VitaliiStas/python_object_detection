...
def stop_tracking():
    global tracker, tracking_initialized, track_all_objects
    tracker = None
    tracking_initialized = False
...
def initialize_tracker(center, size):
    ...
    if rect_start != rect_end:
        # Use TrackerCSRT instead of TrackerKCF
...
def draw_top_text(frame, text, color=(0, 255, 0)):
    cv2.putText(frame, text, (80, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
    ...
    while True:
        ...
        if new_frame_time != prev_frame_time:
            ...
            if tracking_initialized:
                ...
                if success:
                    # Draw top text
                    draw_top_text(display_frame, "Press 'C' to stop tracking",
                                  color=(0, 255, 0))
